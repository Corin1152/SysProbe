...
| **Smart charge** | 充电控制（启停阈值、温度限制、高级项）与电池信息（健康度、循环次数、容量、电压电流） |
...
- 本地直接 `xcodebuild` 时包里**没有**这个工具，那一区会禁用并变淡；具体原因看诊断区的
  **Root tool** 一行，点下去失败时会弹一句「维护工具不在这一版里，或者没能取得 root 权限」。
